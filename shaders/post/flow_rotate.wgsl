// Post-processing: slow rotation distortion.
// Rotates the frame slowly around center, speed driven by amplitude.
// Creates kaleidoscopic trailing patterns.
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Apply 2D rotation around center
    let rotated = vec2<f32>(
        centered.x * ca - centered.y * sa,
        centered.x * sa + centered.y * ca
    );

    // Add subtle inward pull to prevent content drifting to edges
    let pull = -0.002 * (1.0 + u.bass * 0.5);
    let flow = rotated - centered + centered * pull;
