// Mandelbox Flythrough — endless raymarched cathedral of folded boxes and
// spheres. The camera flies forward through the lattice of tunnels the
// box/sphere folds carve out; bass morphs the fold scale so the architecture
// breathes, mids cycle the palette, beats surge the fly speed and glow.
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (hit) {
        ...
        let base = palette(trap * 0.5 + u.time * 0.04 + u.mid * 0.3);
        let l = normalize(vec3<f32>(0.5, 0.7, -0.5));
        let diff = max(dot(n, l), 0.0);
        let fres = pow(1.0 - max(dot(n, view), 0.0), 2.5);

        // AO from step count makes deep folds dark for contrast.
        let ao = 1.0 - steps / 120.0;
    ...
    // Light depth fade into a near-black void so the structure pops.
